...
contract ArraysMappingsAndStructs {
    ...
    mapping(address => Payout[]) internal payoutMapping;
     ...
     */
    function registerUser(string calldata _name, uint256 _walletBalance, address _userWalletAddress) public returns(uint256) {
        require(!isRegistered_ByAddress[_userWalletAddress], "A user already exist with this wallet address. Duplicate users not allowed");
    ...
    function updateUser(
        ...
        require(isRegistered_ById[_id], "A registered user with this id not found or does not exist");
     ...
     */
    function getUserAccountData(address _userWalletAddress) public view returns(User memory) {
        require(isRegistered_ByAddress[_userWalletAddress], "A user with this wallet address not found or does not exist");
    ...
    function recordPayOut(
        ...
        Payout[] storage adminPayoutsToUpdate = payoutMapping[_paidBy];
     ...
     */
    function getAllUserPayouts(address _userWalletAddress) public view returns(Payout[] memory) {
        require(isRegistered_ByAddress[_userWalletAddress], "A user with this wallet address not found or does not exist");
